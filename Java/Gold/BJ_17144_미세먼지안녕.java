...
//		System.out.println(sumDusts());
	...
	private static void purifyUpper() {
		...
		int tmp = map[puriBot-1][c-1]; // 윗부분의 오른쪽아래 부분 
		for(int i = c-1; i> 1; i--) {
			map[puriBot-1][i] = map[puriBot-1][i-1]; // 갱신 
		}
		map[puriBot-1][1] = 0; // 새로운 바람이 부니까 그건 0임
		
		int tmp2 = map[0][c-1]; // 윗부분의 오른쪽 윗부분
		for(int i =0 ; i <puriBot-1 ; i++) {
			map[i][c-1] = map[i+1][c-1];
		}
		map[puriBot-2][c-1] = tmp;
		
		tmp = map[0][0];
		for(int i = 0 ; i < c-2 ; i++) { // 윗라인 교체
			map[0][i] = map[0][i+1];
		}
		map[0][c-2] = tmp2; 
		
		for(int i = 2 ; i< puriBot-1; i++) {
			map[i][0] = map[i-1][0];
		}
		map[1][0] = tmp;
	...
	private static void purifyBelow() {
		if(map[puriBot+1][0] != 0 ) {
			totalDust -=map[puriBot+1][0]; 
		}
		int tmp = map[puriBot][c-1]; // 아래부분의 오른쪽위 부분 
		for(int i = c-1; i> 1; i--) {
			map[puriBot][i] = map[puriBot][i-1]; // 갱신 
		}
		map[puriBot][1] = 0; // 새로운 바람이 부니까 그건 0임
		
		int tmp2 = map[r-1][c-1]; // 아래부분의 오른쪽 아래부분
		for(int i = r-1 ; i >puriBot ; i--) {
			map[i][c-1] = map[i-1][c-1];
		}
		map[puriBot+1][c-1] = tmp;
		
		tmp = map[r-1][0];
		for(int i = 0 ; i < c-2 ; i++) { // 윗라인교체
			map[r-1][i] = map[r-1][i+1];
		}
		map[r-1][c-2] = tmp2; 
		
		for(int i = puriBot+1 ; i < r-2; i++) {
			map[i][0] = map[i+1][0];
		}
		map[r-2][0] = tmp;
	...
	private static void spreadDust() {
		...
		for(int i = 0; i< r ; i++) {
			for(int j = 0 ; j< c; j++) {
				if( map[i][j] <5 )continue; // 5보다 작으면 퍼지지 않아
